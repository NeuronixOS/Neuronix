...
# Generated lists live under /tmp (NEURONIX_LIST_DIR), not neuronix-iso/overlay/.
MANIFEST="${REPO_ROOT}/default/install-list"
STAGING_SH="${REPO_ROOT}/share/neuronix-build-staging.sh"
...
regen_package_lists() {
	local pkg sec
	local desktop_pkgs="${CALAMARES_GEN}/neuronix-desktop-packages.list"
	local server_pkgs="${CALAMARES_GEN}/neuronix-server-packages.list"
	local live_purge="${CALAMARES_GEN}/neuronix-live-purge.list"
	...
	mkdir -p "$LIST_DIR" "$CALAMARES_GEN"
...
# Regenerated by build.sh." \
	...
	_info "wrote ${_dim}${server_pkgs}${_reset}"
	...
	_info "wrote ${_dim}${desktop_pkgs}${_reset}"
	...
	_info "wrote ${_dim}${live_purge}${_reset}"

	rm -f "${CALAMARES_GEN}/neuronix-desktop-purge.list"
}
...
main() {
	...
	if [[ ! -r "$STAGING_SH" ]]; then
		_die "Missing build staging helper: share/neuronix-build-staging.sh"
	fi
	...
	export NEURONIX_LIST_DIR NEURONIX_CALAMARES_GEN NEURONIX_GEN_DIR
	local LIST_DIR="$NEURONIX_LIST_DIR"
	local CALAMARES_GEN="$NEURONIX_CALAMARES_GEN"
	...
	_msg "  Generated:    ${_dim}${NEURONIX_GEN_DIR}${_reset}"
	...
	( cd "$ISO_DIR" && NEURONIX_BUILD_ROOT="$build_root" ./setup.sh )
