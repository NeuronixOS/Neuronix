...
def _powertop_bin() -> str | None:
    """Resolve powertop (lives in /usr/sbin; pkexec secure PATH often omits it)."""
    found = shutil.which("powertop")
    if found:
        return found
    found = shutil.which("powertop", path="/usr/sbin:/usr/bin:/bin")
    if found:
        return found
    for cand in ("/usr/sbin/powertop", "/usr/bin/powertop"):
...
class _PowertopThread(QThread):
    ...
    def run(self):
        bin_path = _powertop_bin()
        if not bin_path or not shutil.which("pkexec"):
            self.done.emit([], [], "powertop or pkexec not found")
            return
        try:
            r = subprocess.run(
                ["pkexec", bin_path, f"--csv={_POWERTOP_CSV}", "--time=3"],
